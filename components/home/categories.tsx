...
const Categories = () => {
    return (
        <VerticalScrollWrapper heading="Explore Categories" icon="compass">
            {categories.map(item =>
                <Link href={`/${item.href}`} className={`card-width p-4 rounded-2xl border flex flex-col gap-4 shadow-md transition-transform animate-duration-transition hover:scale-down ${colors[item.color as Color].border} ${colors[item.color as Color].bgTransparent}`}>
